...
Wandering traders open a custom shop GUI instead of their vanilla trades. The stock is shared globally, persists across restarts, and starts empty — it is filled by players selling items via the Sell items button. Any item except emeralds is accepted. Prices start at 2 emeralds per item and drift with supply and demand (every 32 net items bought or sold shifts the price by one emerald, clamped to 1-64); selling pays out at half the current price. Left-click buys one unit, middle-click ten, right-click a hundred (capped by stock and balance).
